...
const BUFFER_PADDING = {
  CONTROL: intEncoder.encode8Bit(255),
  NULL: 0x00,
};

function checkCommand(str): Command {
  switch (str) {
    case "OK":
      return str;
    case "IN":
...
export const MidiMessage = {
  ...
  encodeSession(params: { command: "IN" | "OK" | "NO"; name: string }) {
  ...
  decodeSessionTimestamp(
    ...
    const { number: timestamp0, unit8Array: buf6 } = decodeAndPopInt64Bit(buf5);
  ...
  decodeSession(messageBuffer: Uint8Array<ArrayBuffer>): DecodedSessionMessage {
    const { unit8Array: buf1 } = decodeAndPopInit8Bit(
      Uint8Array.from(messageBuffer)
    );
    const { unit8Array: buf2 } = decodeAndPopInit8Bit(buf1);
